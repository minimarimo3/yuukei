...
public class ExplorerItemScanner : MonoBehaviour
{
    ...
    private string scannerExePath;

    void Start()
    {
        // プラットフォーム間のセパレータの違いを吸収しつつ、絶対パスを構築
        scannerExePath = Path.GetFullPath(Path.Combine(
            Application.streamingAssetsPath, 
            "ExplorerItemScanner", 
            "ExplorerItemScanner.exe"
        ));

        if (!File.Exists(scannerExePath))
        {
            UnityEngine.Debug.LogError($"実行ファイルが見つかりません: {scannerExePath}\nAssets/StreamingAssets/ExplorerItemScanner/ExplorerItemScanner.exe が配置されているか確認してください。");
            return;
        }
    ...
    void StartScannerProcess()
    {
        ...
        ProcessStartInfo psi = new ProcessStartInfo
        {
            ...
            WorkingDirectory = Path.GetDirectoryName(scannerExePath), // 作業ディレクトリをexeの場所に明示的に設定
